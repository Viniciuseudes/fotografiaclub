...
export default function FormPage() {
  ...
  const [isLoadingImage, setIsLoadingImage] = useState(false); // Novo estado para loading do redimensionamento
  ...
  const resizeImage = (file: File): Promise<File> => {
    return new Promise((resolve, reject) => {
      if (file.size <= MAX_FILE_SIZE_BYTES) {
        console.log(
          `Arquivo "${file.name}" já está abaixo do limite (${(
            file.size /
            1024 /
            1024
          ).toFixed(2)} MB).`
      ...
      console.log(
        `Redimensionando arquivo "${file.name}" (Tamanho original: ${(
          file.size /
          1024 /
          1024
        ).toFixed(2)} MB)...`
          ...
          const attemptResize = () => {
            ...
            canvas.toBlob(
              (blob) => {
                ...
                console.log(
                  ` - Tentativa com quality ${quality.toFixed(2)}: ${(
                    blob.size /
                    1024 /
                    1024
                  ).toFixed(2)} MB`
                ...
                if (blob.size <= MAX_FILE_SIZE_BYTES) {
                  const resizedFile = new File([blob], file.name, {
                    type: file.type.startsWith("image/png")
                      ? "image/png"
                      : "image/jpeg",
                    lastModified: Date.now(),
                  });
                  console.log(
                    ` -> Sucesso! Tamanho final: ${(
                      resizedFile.size /
                      1024 /
                      1024
                    ).toFixed(2)} MB`
                  ...
                  console.log("Qualidade baixa, reduzindo dimensões...");
                  width *= 0.8;
                  ...
                  if (width < 300) {
                    reject(
                      new Error(
                        "Não foi possível reduzir a imagem o suficiente sem perda excessiva de qualidade."
                      )
                    );
                    return;
                  }
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files?.[0]) {
      ...
      if (file.type.startsWith("image/")) {
        setIsLoadingImage(true); // Inicia loading
        try {
          const resizedFile = await resizeImage(file); // Chama a função de redimensionar
          setFormData((prev) => ({
            ...prev,
            photos: [resizedFile], // Substitui com o arquivo redimensionado (ou original)
          }));
        } catch (error) {
          console.error("Erro ao redimensionar imagem (drop):", error);
          alert(
          ...
          setIsLoadingImage(false); // Finaliza loading
        }
      } else {
    ...
    if (e.target.files?.[0]) {
      ...
      if (file.type.startsWith("image/")) {
        ...
        try {
          ...
          console.error("Erro ao redimensionar imagem (input):", error);
          alert(
            `Erro ao processar imagem: ${
              error instanceof Error ? error.message : "Erro desconhecido"
            }`
  ...
  const handleSubmitPhotos = async () => {
    ...
    if (!canProceedStep2) {
      alert("É necessário anexar 1 foto.");
      return;
    }
    ...
    try {
      ...
      console.error("[v0] Erro completo ao enviar fotos:", error);
      alert(
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="space-y-4 pointer-events-none">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {" "}
                        {!isLoadingImage && "ou clique para selecionar"}{" "}
                      </p>{" "}
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {" "}
                      PNG, JPG (máx ~4MB após otimização){" "}
                    </p>
                  </div>
                </div>
                {/* Preview da Foto */}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-sm font-medium text-foreground">
                        ...
                        {(formData.photos[0].size / 1024 / 1024).toFixed(2)} MB){" "}
                      </p>{" "}
                    </div>
                ...
                <div className="flex gap-4">
                  ...
                  <Button
                    ...
                    disabled={isSubmittingPhotos || isLoadingImage}
                  >
                    {" "}
                  ...
                  <Button
                    onClick={handleNext}
                    disabled={
                      !canProceedStep2 || isSubmittingPhotos || isLoadingImage
                    }
                    size="lg"
